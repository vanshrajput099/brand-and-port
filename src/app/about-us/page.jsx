...
const page = () => {
    return (
        <>
            ...
            <div className='w-full lg:w-[80%] mx-auto py-10 flex flex-col sm:flex-row justify-between body-text'>
                <img className='h-[450px] w-[400px] mx-auto sm:w-1/2 sm:h-fit' src={about1.src} alt="" />
                <div className='space-y-5 px-2 w-[90%] mx-auto lg:mx-0 lg:w-1/2 py-10'>
                    <h1 className='font-medium text-xl md:text-3xl xl:text-5xl'>From factory floors to global shelves</h1>
                    <h1 className='orange-text font-medium text-base md:text-xl xl:text-3xl'>we don’t just build brands, we take them places.</h1>
                    <p className='text-base lg:text-xl'>With an extensive experience in logistics, great minds thought of some potential endless ways of living a life with sustainable products. Born in Canada, where creativity collided with commerce, The Brand & Port was founded on a bold idea: that branding and global sourcing could become the ultimate partnership of vision and execution.
                        At The Brand & Port, we navigate the intricate worlds of branding and international business with precision. </p>
                    <p className='text-base lg:text-xl'><span className='orange-text'>Our purpose?</span> To empower brands not just to look exceptional  but to perform powerfully across markets. We believe a great brand is like a masterfully built ship: enduring, elegant, and ready to chart new horizons.
