...
const HowWeWorkComponent = ({ data, idx }) => {
    ...
    return (
        <div ref={ref} className={`flex transition-all duration-1000 delay-300 ease-out transform ${idx % 2 !== 0 ? `flex-col md:flex-row-reverse ${inView ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-100'}` : `flex-col md:flex-row ${inView ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-100'}`} gap-5 md:gap-0  md:justify-between`}>
