...
const AboutUs = () => {
    ...
    return (
        <div className='space-y-5 w-[80%] px-5 xl:px-0 xl:w-[60%] mx-auto xl:w-container py-10'>
            <h1 ref={titleRef} className={`heading-text text-3xl md:text-5xl xl:text-7xl bg-gradient-to-r from-white via-black via-80% xl:via-50% to-black bg-clip-text text-transparent transition-all duration-1000 delay-200 ease-out transform ${titleInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>About Us</h1>
            <div ref={descRef} className={`body-text space-y-2 xl:space-y-10 text-xs md:text-base xl:text-xl mt-5 xl:mt-10 transition-all duration-1000 delay-400 ease-out transform ${descInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <img ref={imageRef} src={aboutUsGroup.src} className={`mt-10 md:mt-15 xl:mt-20 transition-all duration-1000 delay-200 ease-out transform ${imageInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} alt="" />
        </div>
    )
}
