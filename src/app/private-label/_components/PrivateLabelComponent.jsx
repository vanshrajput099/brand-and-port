...
const PrivateLabelComponent = ({ data, idx }) => {
    ...
    return (
        <div ref={ref} className={`flex flex-col items-center md:flex-row transtion-all duration-1000 delay-100 ease-out transform ${idx % 2 !== 0 ? `md:flex-row-reverse ${inView ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-90'}` : `${inView ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-90'}`} justify-between space-y-5`}>
